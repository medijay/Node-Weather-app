...
button.addEventListener('click', () => {
    ...
    .then(resp => {
        if (!searchArea) {
            ...
            document.querySelector('.weather')[0].appendChild(location);
            document.querySelector('.weather')[0].innerHTML += resp.location;

            document.querySelector('.weather')[1].appendChild(location);
            document.querySelector('.weather')[1].innerHTML += resp.forecast;
        }
    })
});